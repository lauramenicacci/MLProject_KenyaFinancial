...
We figured that, by employing some of the usual of machine learning, such as logistic regression, ridge regression, Decision Trees, support vector machines, and Random Forests, we could find a classification algorithm for best predicting modes of transactions, in terms of whether mobile money was used or not. Furthermore, the machine learning algorithms named above are interpretable! That means, we could use them to understand which feature are important to predict the usage of mobile money.
...
```{r fig2, eval = TRUE, echo = FALSE, out.width = '65%', fig.cap = "Decision Tree performance", fig.align = 'center'}
...
```{r fig4, eval = TRUE, echo = FALSE, out.width = '70%', fig.cap = "ROC-AUC Curve for the best two models with a selected sample", fig.align = 'center'}
...
one of these classes, mobile money, as such class has always represented the goal of our research question. We plotted the ROCAUC curve for our two best models, namely the Random Forest with 25 trees and the Decision Tree with a depth of
50. We chose the best performing decision tree, and the number of tree that acted like a cutoff in our GridSearch
training for such models. We observed indeed that after 25 trees, the model wasn’t improving significantly as before.
7
As plotted in Table 2, the performance of these two models, according to the ROC-AUC score, is clearly the best
one. These two algorithms indeed were able to gain a high score even with such a strict metric. The ROC-AUC Curve
plotted here reflects such performances also on the prediction of a class that is present in only the 2% of the data.
